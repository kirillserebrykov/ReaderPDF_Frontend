...
import AddBookContainer from "./AddBook/AddFileContainer";
...
const HomeUI = ({
	...
	const Cards =
		...
		dataCatalog.map(el => {
			return (
				<div key={el._id} className={style.cardWrap}>
					<CardsFile
	...
	return (
		<>
			...
			<section className={style.wrapperHome}>{Cards && Cards}</section>
			...
			<TransitionGroup>
				<Routes>
					<Route
						path=":StatusUploadFile/*"
						element={<AddBookContainer refetchCatalog={refetchCatalog} />}
					/>
				</Routes>
			</TransitionGroup>
		</>

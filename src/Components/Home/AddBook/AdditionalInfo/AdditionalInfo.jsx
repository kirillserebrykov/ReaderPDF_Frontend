			...
			<CustomTextField
				...
				{...register(NameTextField, {
					...
					maxLength: {
						...
						message: `Максимум допустимое кол-во символов ${maxLengthValue}`,
					},
				})}
				label={NameTextField}
				variant="standard"
		...
		<>
			<form onBlur={handleSubmit(onSubmit)}>
				<fieldset className={style.InputSet}>
					<legend className={style.InputSetName}>
						{" "}
						<span>{FileName}</span>{" "}
					</legend>
					<BtnDeleteContainer
					...
					<ThemeProvider theme={outerTheme}>
						<ValidTextField
							defaultValue={FileName && FileName.replace(".pdf", "")}
							NameTextField="Name"
							maxLengthValue="25"
						/>
						<ValidTextField NameTextField="Author" maxLengthValue="25" />
						<ValidTextField
							NameTextField="Description"
							maxLengthValue="160"
							multiline
							maxRows={4}
						/>
...
const AdditionalInfo = ({ stateAddition, stateForms, setStateForms }) => {
	const FormForFile = useMemo(() => {
		return stateAddition.map((file, index) => (
			<Form key={index} stateForms={stateForms} setStateForms={setStateForms} file={file} />

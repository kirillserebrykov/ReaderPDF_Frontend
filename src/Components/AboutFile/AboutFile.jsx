...
const AboutFileUI = ({ metadata, RedirectToReadFile, Name, isLoading }) => {
	return (
		<>
			{isLoading ? (
				<Header name={Name ? Name : "Загрузка..."} back_button={true} />
			) : (
				<Header name={Name ? Name : "Ошибка"} back_button={true} />
			)}
			{!isLoading ? (
				<section className={style.wrapperFile}>
					<div className={style.cover}>
						<img src={`http://localhost:5000/cdn/CoverFile?fileName=${Name}.pdf`} alt="" />
						<div className={style.readBtnWrap}>
							<button onClick={RedirectToReadFile} className={style.readBtn}>
								<span>Читать</span>
							</button>
						</div>
					</div>
	...
	return (
		<>
			{error ? (
				...
				<AboutFileUI
					metadata={metadata}
					RedirectToReadFile={RedirectToReadFile}
					Name={Name}
					isLoading={isLoading}
				/>
			)}
